...
const SubCategories = (props) => {
  ...
  const getData=()=>{
      ...
      api.common('category/list?size=10000').getAll().then(({ data }) => {
        if(data.success == true){
          ...
          console.log('cates',categoriesList);
        }
      })
  ...
  const getSingleData=(id)=>{
    ...
    api.common('subcategory/list/'+id).getOne().then(({ data }) => {      
      ...
      console.log('single sub cat',singleSubCategory);
  ...
  const deleteSingleData=(id)=>{
    ...
    if (window.confirm("Are you sure do you want to delete the sub category?")) {
      ...
      api.common('subcategory/delete/'+id).getOne().then(({ data }) => {  
  ...
  const submitData=(formData)=>{    
    ...
    let url='subcategory/add';
    if(Object.keys(singleSubCategory).length > 0){
      url='subcategory/list/'+singleSubCategory._id;
    }
  ...
  console.log({data});
    if(data.length == 0){
      ...
      return data.map((data, index) => {
        const { subcategory_name, subcategory_desc,_id,category_name } = data //destructuring
        return (
           <tr key={index}>
              <td>{parseInt(newIndex)+parseInt(index)+1}</td>
              <td>{category_name}</td>
              <td>{subcategory_name}</td>
              <td>{subcategory_desc}</td>
              ...
              <div className="modal-body p-0">
                <Card className="bg-secondary shadow border-0">
                  ...
                  <CardBody className="px-lg-5">                   
                      ...
                      <SubCategory submitData={submitData} singleSubCategory={singleSubCategory} categoriesList={categoriesList} />
                  </CardBody>
                </Card>
              </div>
            </Modal>
