...
const Roles = (props) => {
  ...
  const getData=()=>{
      api.common('roles/list?page='+currentPage).getAll().then(({ data }) => {
  ...
  const getSingleData=(id)=>{
    api.common('roles/list/'+id).getOne().then(({ data }) => {
      console.log('value',data);

      if(data.success == true){
        setSingleRole(data.data);
        setFormModal(!formModal)  
      }
  ...
  const deleteSingleData=(id)=>{
    ...
    if (window.confirm("Are you sure do you want to delete the role?")) {

      api.common('roles/delete/'+id).getOne().then(({ data }) => {  
  ...
  const submitData=(formData)=>{    
    ...
    let url='roles/add';
    if(Object.keys(setSingleRole).length > 0){
      url='roles/list/'+setSingleRole._id;
    }
  ...
  const renderTableTr=(data)=>{
    ...
    if(data.length == 0){
      ...
      return data.map((data, index) => {
        const { title,_id } = data //destructuring
              ...
              <td className="text-center">
                      <UncontrolledDropdown>
                        ...
                        <DropdownMenu className="dropdown-menu-arrow" right>
                          ...
                          <DropdownItem
                            href={"/admin/user-privileges/"+_id}
                ...
                <Card className="bg-secondary shadow border-0">
                  <CardHeader className="bg-transparent">
                    <div className="text-muted text-center mt-2 mb-3">
                      <small>
                        {
                          Object.keys(singleRole).length > 0?'Edit ':'Add new '
                        }
                        Role</small>
                    </div>
                  ...
                  <CardBody className="px-lg-5">                   
                      ...
                      <Role submitData={submitData} singleRole={singleRole} />
                  </CardBody>
                </Card>
              </div>
            </Modal>
